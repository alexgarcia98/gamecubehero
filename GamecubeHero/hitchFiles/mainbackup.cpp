...
int main(int argc, char const** argv) {
    ...
    while (window.isOpen()) {
        ...
        while (window.pollEvent(event)) {
            ...
            */
            ...
            if(event.type == sf::Event::JoystickButtonPressed) {
                ...
                switch(event.joystickButton.button) {
                    case 11:
                        point = noteManager.onInputDown(UP);
                        break;
                    case 10:
                        point = noteManager.onInputDown(RIGHT);
                        break;
                    case 9:
                        point = noteManager.onInputDown(DOWN);
                        break;
                    case 8:
                        point = noteManager.onInputDown(LEFT);
                        break;
                    case 6:
                        point = noteManager.onInputDown(L2);
                        break;
                    case 7:
                        point = noteManager.onInputDown(R2);
                        break;
                    case 4:
                        point = noteManager.onInputDown(L1);
                        break;
                    case 5:
                        point = noteManager.onInputDown(R1);
                        break;
                    case 3:
                        point = noteManager.onInputDown(TRIANGLE);
                        break;
                    case 2:
                        point = noteManager.onInputDown(CIRCLE);
                        break;
                    case 1:
                        point = noteManager.onInputDown(X);
                        break;
                    case 0:
                        point = noteManager.onInputDown(SQUARE);
                        break;
                }
            }
        ...
        if(noteManager.getFirst()!=nullptr) {
            ...
            if(100 - point > 0) {
                score = score + (mult * (100 - point));
                combo = combo + 1;
            }
            if(mult < 4) {
                mult = (combo / 10) + 1;
            }/*
            if(noteManager.getFirst()->getPos().y < 100) {
                std::cout
                combo = 99;
            }*/
            //std::cout<<
            text.setString("Score: " + std::to_string(score) + " | Combo: " + std::to_string(combo) + " | Mult: " + std::to_string(mult) + "x");
            point = 100;
        }
        window.draw(text);
